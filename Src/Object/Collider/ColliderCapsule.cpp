...
VECTOR ColliderCapsule::GetPosOnLine(float rate) const
{
	rate = SchoolUtility::Clamp(rate, 0.0f, 1.0f);
	return SchoolUtility::Lerp(GetPosDown(), GetPosTop(), rate);
}
...
bool ColliderCapsule::IsHitPoint(const VECTOR& pos) const
{
	// カプセルの線分両端（ワールド座標）
	VECTOR a = GetPosDown();
	VECTOR b = GetPosTop();

	// 線分 ab と点 pos の最短距離を求める
	VECTOR ab = VSub(b, a);
	VECTOR ap = VSub(pos, a);

	float abLen2 = VDot(ab, ab);
	// 線分が縮退している（長さゼロ）の場合は球との判定にフォールバック
	if (abLen2 <= 0.0f)
	{
		VECTOR diff = VSub(pos, a);
		float dist2 = VDot(diff, diff);
		return dist2 <= radius_ * radius_;
	}

	// 線分上の最近接点のパラメータ t を求め、0..1 にクランプ
	float t = VDot(ap, ab) / abLen2;
	t = SchoolUtility::Clamp(t, 0.0f, 1.0f);

	// 最近接点
	VECTOR closest = VAdd(a, VScale(ab, t));

	// 点と最近接点の距離の二乗と半径の二乗を比較
	VECTOR diff = VSub(pos, closest);
	float dist2 = VDot(diff, diff);

	return dist2 <= radius_ * radius_;
}

bool ColliderCapsule::IsHit(const ColliderSphere* collider) const
{
	if (collider == nullptr) return false;

	return HitCheck_Sphere_Capsule(collider->GetPos(), collider->GetRadius(),
		GetPosTop(),GetPosDown(),radius_);
}

bool ColliderCapsule::IsHit(const ColliderCapsule* collider) const
{
	if (collider == nullptr) return false;

	return HitCheck_Capsule_Capsule(GetPosTop(), GetPosDown(), radius_,
		collider->GetPosTop(), collider->GetPosDown(), collider->GetRadius());
}
